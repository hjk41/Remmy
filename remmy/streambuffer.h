...
namespace remmy {
    ...
    public:
        ...
        void InitOstream() {
            REMMY_ASSERT(buf_ == nullptr, "trying to init a already-initialized buffer");
        ...
        void DetachBuf(void** buf, size_t* size) {
            *size = GetSize();
            if (const_buf_) {
                REMMY_WARN("Detaching a constant stream buffer will result in a memory copy");
        ...
        void Write(const void * buf, size_t size) {
            REMMY_ASSERT(!const_buf_, "writing into a const buffer is not allowed.");
            if (buf_ == nullptr) {
                InitOstream();
            }
            ...
            if (new_size > pend_) {
                // reallocate buffer
                REMMY_LOG("buffer is full, reallocating. pend_ = %d, new_size = %d", pend_, new_size);
                new_size = std::max(new_size, ppos_ + GROW_SIZE);
                char * new_buf = (char *)realloc(buf_, new_size);
                REMMY_ASSERT(new_buf, "realloc failed");
                buf_ = new_buf;
                pend_ = new_size;
            }
        ...
        void WriteHead(const char * buf, size_t size) {
            ...
            if (gpos_ < size) {
                ...
                REMMY_WARN("reallocating due to write_head, possible performance loss. gpos_ = %d, size = %d", gpos_,
                           size);
    ...
    public:
        ...
        void Resize(size_t size) {
            ...
            REMMY_ASSERT(newbuf != nullptr, "realloc failed, original size=%lld, target size=%lld", size_, size);
            buf_ = newbuf;
            size_ = size;
        }
        ...
        void Compact(uint64_t offset) {
            REMMY_ASSERT(offset <= received_bytes_,
